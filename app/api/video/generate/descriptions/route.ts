...
import { buildDescriptionsPrompt } from "@/lib/llm/video-prompts"
...
export async function POST(req: NextRequest) {
  ...
  if (project.googleDriveTokens && project.googleDriveFolderId) {
    try {
      const doc = await createVideoGoogleDoc(
        videoProjectId,
        `${project.name} Video Descriptions`,
        htmlContent,
      ...
      // Update Sheet Column E with doc URL if sheet exists
      if (project.sheetId && docUrl) {
        await writeSheetCell(videoProjectId, project.sheetId, "E", 2, docUrl)
      }
    } catch {
      // Non-fatal: descriptions content still returned
    }
  }
  ...
  if (docId && docUrl) {
    await prisma.videoProject.update({
      where: { id: videoProjectId },
      data: { descriptionsDocId: docId, descriptionsDocUrl: docUrl },
    })
  }

  // Update all topics to COMPLETED description status
  await prisma.videoTopic.updateMany({
    where: { videoProjectId },
    data: { descriptionStatus: "COMPLETED" },
  })
